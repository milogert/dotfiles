...
Functions.addFunctionalLogger = function(isConditional)
  ...
  local template = "const %s = tag => %slogData => ( console.log(`"
    .. fileName
    .. "%s"
    .. tail
  local predicateArg = u.tern(isConditional, "predicate => ", "")
  local predicateLog = u.tern(
    isConditional,
    ": CONDITION ${predicate(logData) ? '' : 'NOT '}MET",
    ""
  )
...
Functions.addNonConditionalLogger = function()
  Functions.addFunctionalLogger(false)
end
Functions.addConditionalLogger = function()
  Functions.addFunctionalLogger(true)
end
...
Functions.prettyPrintJsonFile = function()
  Functions.prettyPrintJson(true)
end
Functions.prettyPrintJsonVisual = function()
  Functions.prettyPrintJson(false)
end
...
Functions.twiddleCase = function(str)
  ...
  elseif is_lower then
    local match, _ = string.gsub(str, "(%a)(%a*)", function(head, rest)
...
-- TODO this requires toggleterm, which I do not want to install.
Functions.toggleSlumber = function()
